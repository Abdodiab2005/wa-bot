...
async function connectToWhatsApp() {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    if (confirmationSessions.has(senderId) && body.toLowerCase() === "yes") {
      ...
      confirmationSessions.delete(senderId); // Use the confirmation once and delete it

      if (session.command === "removeall") {
        // This is where our new function is called
        await executeRemoveAll(sock, session);
      }
      return; // Important: Stop further processing after handling the confirmation
    }
    const prefix = "!";
...
// A possible implementation of your normalizeJid function
